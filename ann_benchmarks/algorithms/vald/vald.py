...
from vald.v1.vald import insert_pb2_grpc, search_pb2_grpc, object_pb2_grpc
...
grpc_opts = [
    ('grpc.keepalive_time_ms', 1000 * 10), ('grpc.keepalive_timeout_ms', 1000 * 10),
...
class Vald(BaseANN):
    ...
    def fit(self, X):
        ...
        astub.CreateIndex(
            payload_pb2.Control.CreateIndexRequest(
                pool_size=100000))
        time.sleep(5)
        print(astub.IndexInfo(payload_pb2.Empty()))

        ostub = object_pb2_grpc.ObjectStub(channel)
        oreqs = [payload_pb2.Object.ID(id=str(i)) for i, _ in enumerate(X)]
        for _ in ostub.StreamGetObject(iter(oreqs)):
            pass
    ...
    def query(self, v, n):
        cfg = payload_pb2.Search.Config(num=n, radius=2.0, epsilon=self._epsilon, timeout=3000000)
